...
/**
 * Testing the quasi-randomness realization used for Low-discrepancy sequence generation.
 */
...
/**
 * Class QuasiRandomTest - Testing the quasi-randomness class.
 *
 * @package CryptoManana\Tests\TestSuite\Randomness
 */
final class QuasiRandomTest extends AbstractUnitTest
{
    /**
     * How many times to rerun generation tests.
     */
    const REPEATED_TESTS_COUNT = 2;

    /**
     * Creates new instances for testing.
     *
     * @return QuasiRandom Testing instance.
     */
    private function getRandomnessSourceForTesting()
    {
        return new QuasiRandom();
    }
    ...
    public function testSeedingAndAutoSeedingActions()
    {
        ...
        $currentSeed = $reflection->getValue();

        // Seeding test for producing the same values
        $randomness::setSeed($currentSeed);
        $numberOne = $randomness->getInt();

        $randomness::setSeed($currentSeed);
        $numberTwo = $randomness->getInt();

        $this->assertEquals($numberOne, $numberTwo);

        // Seeding test for not producing the same values
        $numberOne = $randomness->getInt();

        $randomness::setSeed($currentSeed * -1);
        $numberTwo = $randomness->getInt();
    ...
    public function testSupportedRangeForIntegerGeneration()
    {
        ...
        $this->assertEquals(QuasiRandom::QUASI_INT_MAX, $randomness->getMaxNumber());
        $this->assertEquals(QuasiRandom::QUASI_INT_MIN, $randomness->getMinNumber());
    ...
    public function testByteAndBooleanGeneration()
    {
        ...
        for ($i = 1; $i <= self::REPEATED_TESTS_COUNT; $i++) {
            $tenBytes = $randomness->getBytes(512); // doubles the set

            $this->assertTrue(!empty($tenBytes) && is_string($tenBytes));
            $this->assertTrue(strlen($tenBytes) === 512);
